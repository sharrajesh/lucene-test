...
public class IndexConfig {
	...
	private static void ParseConfigXml(String configFile) throws Exception {
		...
		try {
			...
			Parsed.INPUT_FILES_DIR = config.getString("params.input_dir")
			    .toLowerCase();
			Parsed.WIKI_LINE_FILE_NAME = config.getString("params.input_file")
			    .toLowerCase();
			...
			Parsed.INDEX_CLOSE_THRESHHOLD = config
			    .getInt("params.index_close_threshhold");
			...
			Parsed.INDEXING_SCHEME = config.getString("params.index_scheme")
			    .toLowerCase();
	...
	private static void ValidateParsedConfig() throws Exception {
		...
		if (Parsed.WIKI_LINE_FILE_NAME != null
		    && !FileExists(Parsed.WIKI_LINE_FILE_NAME))
	...
	private static void PrintConfig() {
		...
		Log.info("WIKI_LINE_FILE: "
		    + (Parsed.WIKI_LINE_FILE_NAME == null ? "Not in use"
		        : Parsed.WIKI_LINE_FILE_NAME));
		Log.info("INPUT_DIR: " + Parsed.INPUT_FILES_DIR);
		Log.info("OUTPUT_DIR: " + Parsed.OUTPUT_DIR + " OUTPUT_DIR_FINAL: "
		    + Parsed.OUTPUT_DIR_FINAL + " OUTPUT_DIR_TEMP: "
		    + Parsed.OUTPUT_DIR_TEMP);

		Log.info("MAX_MEMORY: " + MAX_MEMORY + " TOTAL_MEMORY: " + TOTAL_MEMORY
		...
		Log.info("TOTAL_DOCS_TO_INDEX: " + TOTAL_DOCS_TO_INDEX
		    + " INPUT_DATA_SIZE: " + INPUT_DATA_SIZE + " DATA_PER_WORKER: "
		    + dataPerWorker);
	}
	...
	private static void FindFilesToIndex() {
		if (UsingWikiLineFile()) {
			...
			FILES_TO_INDEX = FileUtils.listFiles(inputDir,
			    IndexConfig.EXTENSIONS_TO_INDEX, true);
	...
	private static void SetThreshholds() throws IOException {
		final long dataPerWorker = (INPUT_DATA_SIZE + MAX_THREADS) / MAX_THREADS;
		if (Parsed.INDEX_CLOSE_THRESHHOLD == 0
		    || Parsed.INDEX_CLOSE_THRESHHOLD == -1) {
			Parsed.INDEX_CLOSE_THRESHHOLD = (MAX_MEMORY + 3 * MAX_THREADS)
			    / (3 * MAX_THREADS);
