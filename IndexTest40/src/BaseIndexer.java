...
public class BaseIndexer implements Runnable {
	...
	BaseIndexer(IndexWriter mainIndexWriter, BufferedReader wikiReader,
	...
	protected static IndexWriterConfig MakeIndexWriterConfig() throws IOException {
		IndexWriterConfig indexWriterConfig = new IndexWriterConfig(
		    IndexConfig.LUCENE_VERSION, new StandardAnalyzer(
		        IndexConfig.LUCENE_VERSION));
		if (IndexConfig.Parsed.RAM_BUFFER_SIZE_MB != 0)
			indexWriterConfig
			    .setRAMBufferSizeMB(IndexConfig.Parsed.RAM_BUFFER_SIZE_MB);
		if (IndexConfig.Parsed.NO_MERGE_POLICY)
			indexWriterConfig.setMergePolicy(NoMergePolicy.NO_COMPOUND_FILES);
		return indexWriterConfig;
	}

	protected static IndexWriter MakeIndexWriter(Directory outputDir)
	    throws IOException {
	...
	private Document GetNextDocumentFromMainList() throws IOException {
		...
		doc.add(new LongField("date", attributes.creationTime().toMillis(),
		    Field.Store.YES));
	...
	private void PrintStatusUpdate() {
		if (LocalDocsIndexed % IndexConfig.STATUS_UPDATE_THRESHHOLD_DOCS == 0) {
			...
			long totalDocsIndexed = Metrics.TotalDocsIndexed.get();
			Log.info("[" + Thread.currentThread().getId() + "]" 
			    + " Adding TotalDocs: " + ThreadSafeFormatter.Format(totalDocsIndexed) 
			    + " LocalDocs: " + ThreadSafeFormatter.Format(LocalDocsIndexed) 
			    + " TotalBytes: " + ThreadSafeFormatter.Format(totalBytesIndexed) 
			    + " LocalBytes: " + ThreadSafeFormatter.Format(LocalBytesIndexed)
			    + " TotalTimeElapsed: " + ThreadSafeFormatter.Format(totalTimeElapsed));
			Log.info("[" + Thread.currentThread().getId() + "]" 
			    + " GigsPerHour Thread Rate: " + Metrics.GigsPerHour(LocalBytesIndexed, AddDocsWatch.GetElapsed())
	...
	public void run() {
		try {
			...
			Log.error("[" + Thread.currentThread().getId()
			    + "]  exception happend after : " + Metrics.Watch.GetElapsed()
			    + " millisecs");
